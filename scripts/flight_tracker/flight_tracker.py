...
def run_pipeline(start: datetime, stop: datetime) -> int:
    """Run the full pipeline for a time range. Returns number of new flights appended."""
    username = os.environ.get("OPENSKY_USER", "")
    password = os.environ.get("OPENSKY_PASS", "")
    if not username or not password:
        logger.error("Set OPENSKY_USER and OPENSKY_PASS environment variables.")
        raise SystemExit(1)

    # Stage 1: Fetch state vectors from OpenSky
    ...
    # Stage 2: Collapse to one row per flight
    ...
    # Stage 3: Enrich with FR24 route data
    ...
    from processing import _approx_distance_km
    ...
    # Stage 5: Dedup and append to CSV
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    if CSV_PATH.exists():
        existing = pd.read_csv(CSV_PATH, dtype=str)
        # Dedup by callsign + date
...
def main() -> None:
    ...
    logger.info("Running pipeline for %s to %s", start.date(), stop.date())
    total = 0

    # Process one day at a time to stay within OpenSky query limits
    current = start
    while current <= stop:
        day_start = current
        day_stop = current.replace(hour=23, minute=59, second=59)
        count = run_pipeline(day_start, day_stop)
        total += count
