...
- **HA caches the *parsed* lovelace config in server memory** — WS `lovelace/config` and the frontend render return that cache, not disk, for 30+ min after a pull. Force a re-read: `sendMessagePromise({type:'lovelace/config', url_path:'wall-tablet', force:true})`. A no-`force` read silently shows stale config (cache, not a pull problem — pull is on-or-off; see **reload-after-push**).
- **Template *sensors* aren't cached this way** — a chip updates on `template.reload` while a markdown card in the same view stays stale until the parse is re-read. Chip-vs-card disagreement is the tell.
